...
///
/// Since SolSVM doesn't use slots or forks, this implementation is mocked.
pub(crate) struct SolSVMForkGraph {}

impl ForkGraph for SolSVMForkGraph {
...
/// `TransactionBatchProcessor` for use within SolSVM.
///
...
) -> TransactionBatchProcessor<SolSVMForkGraph> {
    let processor = TransactionBatchProcessor::<SolSVMForkGraph>::default();

    {
        let mut cache = processor.program_cache.write().unwrap();

        // Initialize the mocked fork graph.
        cache.fork_graph = Some(Arc::new(RwLock::new(SolSVMForkGraph {})));
...
/// SolSVM, since we don't need to perform such pre-checks.
