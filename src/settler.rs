//! SolSVM's "settler" component for settling the final ledgers across all
...
use {
    crate::transaction::SolSVMTransaction,
...
/// A ledger of SolSVM transactions, used to deconstruct into base chain
/// transactions.
///
    ...
    fn new(
        ...
        solsvm_transactions
            ...
            .for_each(|(transaction, result)| {
                // Only append to the ledger if the SolSVM transaction was
                // successful.
...
/// SolSVM final transaction settler.
pub struct SolSVMSettler<'a> {
    rpc_client: &'a RpcClient,
}

impl<'a> SolSVMSettler<'a> {
    ...
    pub fn process_settle(
        ...
        // Build the ledger from the processed SolSVM transactions.
        let ledger = Ledger::new(solsvm_transactions, svm_output);
